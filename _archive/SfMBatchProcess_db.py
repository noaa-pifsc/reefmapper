    ...
    try:
        ...
        if start_step >= 1 and end_step ==1:
            ...
            try:
                ...
                if os.path.exists(psxfile):
                    logger.info(f"Opening existing project at {psxfile}")
                    doc.open(psxfile)
                    chunk = doc.chunk if doc.chunk else doc.addChunk()
                else:
                    logger.info(f"No project file found at {psxfile}, creating new chunk")
                    chunk = doc.addChunk()

                # --- Gather photo list ---
                photo_list = []
                getPhotoList(os.path.join(root_path, ''), photo_list)
                n_photos = len(photo_list)
                if n_photos == 0:
                    raise Exception(f'No photos found in {root_path}')

                # --- Check which photos are already in the chunk ---
                existing_labels = {cam.label for cam in chunk.cameras}
                new_photos = [p for p in photo_list if os.path.basename(p) not in existing_labels]

                if new_photos:
                    logger.info(f"Adding {len(new_photos)} new photos")
                    chunk.addPhotos(new_photos)
                    doc.save(psxfile)
                    opf.write(f"Added {len(new_photos)} new photos\n")
                else:
                    logger.info("All photos already added — skipping addPhotos")
                    opf.write("Photos already added\n")
                ...
                if n_enabled-counter==0:
                    ...
                    quit()
                # --- Backup project ---
                backup_psx = os.path.join(prod_path, f"{folder_name}_bkup.psx")
                shutil.copyfile(psxfile, backup_psx)
                backup_files = os.path.join(prod_path, f"{folder_name}_bkup.files")
                if os.path.exists(backup_files):
                    shutil.rmtree(backup_files, ignore_errors=True)
                shutil.copytree(os.path.join(prod_path, f"{folder_name}.files"), backup_files)

                logger.info("Step 2 complete")
                opf.write("Step 2 complete\n")

                return chunk, readme_path

            except Exception as e:
                msg = f"Step 2 failed: {e}"
                logger.error(msg)
                opf.write(msg + '\n')
                return
        ...
        if start_step==4 and end_step==4 and job.status =='reviewed':
        ...
        #==============================================================================================
        # Step 7: Build and export interpolated DEM and Orthomosaic
        if start_step >= 6 and end_step == 6 and job.status == 'reviewed':
            db.log_step(job.id, 7, "info", "Loading project")
            print("Step 7. Loading Project")
            doc = Metashape.app.document
            if not hasattr(doc, 'chunk') or doc.chunk is None:
                doc.open(psxfile)
            chunk = doc.chunk
            ...
            # --- Build interpolated DEM ---
            db.log_step(job.id, 7, "info", "Building interpolated DEM")
            chunk.buildDem(
                source_data=Metashape.DenseCloudData,        # use dense cloud
                interpolation=Metashape.EnabledInterpolation
            )

            # --- Build orthomosaic using interpolated DEM ---
            db.log_step(job.id, 7, "info", "Building orthomosaic using interpolated DEM")
            chunk.buildOrthomosaic(
                surface_data=Metashape.ElevationData,       # uses the DEM we just built
                fill_holes=True,
