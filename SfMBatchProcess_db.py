...
from database.db_manager import DatabaseManager, ProcessingJob
...
def MetashapeProcess(job: ProcessingJob):
    ...
    try:
        root_path = job.project_path.replace('\\\\', '\\')  # Handle double backslashes
        ...
        if start_step<=2 and end_step>=2:
            ...
            if n == 0:
                msg = f'No photos found in {os.path.join(root_path, extra)}'
                print(msg)
                logger.warning(msg)
            ...
            if n_enabled-counter==0:
                ...
                msg = "Too many images failed to align"
                db.log_step(job.id, 2, "error", msg)
                logger.error(msg)
                return
...
def main():
    """Main function to process jobs from database."""
    try:
        # Get pending jobs for this batch
        jobs = db.get_pending_jobs(batch_no)
        
        if not jobs:
            logger.info(f"No pending jobs found for batch {batch_no}")
            return
            
        for job in jobs:
            logger.info(f"Processing job {job.id}: {job.site_id}")
